function amplify_spatial_Gdow(vidFile,outDir, rawframes,alpha,level, ...
                     fl,fh,samplingRate, chromAttenuation, fr)
...
%     len = vid.NumberOfFrames;
    ...
    info = imfinfo(vidFile);
    numframe = length(info);
%     for K = 1 : numframe
%         rawframes(:,:,:,K) = imread(vidFile, K);
%     end     
    [vidHeight, vidWidth, nChannels, len] = size(rawframes);
    ...
    Gdown_stack = build_GDown_stack(rawframes, vidHeight, vidWidth, nChannels, startIndex, endIndex, level);
    disp('Finished')
